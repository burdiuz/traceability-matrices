...
```
src/
  cypress/         # Core Cypress integration — feature creation, trace recording, structure
    feature.ts     # createFeature(), FeatureApi, category(), requirement(), trace()
    structure.ts   # Structure tree logic, readStructureRequirements()
    records.ts     # Coverage record accumulation and after() hook
    index.ts       # Public re-exports for /cypress entry point
    types.ts       # Shared TypeScript types
    utils.ts       # Internal helpers
  parsers/         # File parsers (markdown, yaml, json, xml, html)
    markdown.js    # createFeatureFromMarkdown / createFeatureFromMarkdownAsync
    yaml.js        # ...
    json.js
    xml.js
    html.js
  commands/        # CLI command implementations
    serve.ts       # HTTP(S) dev server
    generate-static.ts
    threshold.ts
    stats.ts
    lcov.ts
    scan_features.ts
  view/            # HTML report rendering (pug templates + bundled JS/CSS)
    feature.ts     # Single-feature page
    features.ts    # All-features overview
    page.ts        # Page layout wrapper
    totals.ts      # Aggregated coverage totals
cli.js             # Entry point for the traceability-matrices CLI binary
rollup.config.mjs  # Bundles src/ to root-level dist files
```

**Compiled output files** at the root (`cypress.js`, `markdown.js`, `yaml.js`, etc.) are the published package entry points generated by `npm run build`. Never edit them directly — always edit `src/` and rebuild.
...
```bash
npm run build      # Rollup — rebuilds all entry points from src/
npm test           # Jest
npm run test:watch
```

Tests live in `src/cypress/__tests__/` and `src/parsers/*-test.js`. Cypress specs under `tests/` are integration examples, not part of `npm test`.
...
1. Create `src/commands/<name>.ts` implementing and exporting an async handler.
2. Register it in `cli.js` following the pattern of existing commands.
3. Document flags in the help string.

### Adding a file parser

1. Create `src/parsers/<format>.js` exporting `createFeatureFrom<Format>` and `createFeatureFrom<Format>Async`.
2. Add root-level `<format>.js` and `<format>.d.ts` (copy from an existing pair).
3. Register the extension in `src/commands/scan_features.ts`.
4. Add a `<format>-test.js` in `src/parsers/`.
5. Add the entry to `rollup.config.mjs`.

### Changing the HTML report UI

Pug templates are in `src/view/`. JS and CSS assets are in `src/view/js/` and `src/view/css/`. Run `npm run build` after editing to bundle them into `commands/`.

### Adding structure API methods

Structure logic is in `src/cypress/structure.ts`. Public API surface is re-exported from `src/cypress/index.ts`. TypeScript types are in `src/cypress/types.ts` and `src/cypress/Cypress.d.ts`.

---

## Dependencies policy

- Do **not** use `@actualwave/has-own` or `@actualwave/is-function`. Use `Object.hasOwn(obj, key)` and `typeof value === 'function'` directly.
- Runtime dependencies are kept minimal (`koa`, `@koa/router`, `open`, `pug`). Parser dependencies (`js-yaml`, `node-html-parser`, etc.) are `devDependencies` because Rollup bundles them into the parser entry points.
...
2. `npm run build` — verify compiled output is updated.
3. `npm test` — all tests must pass.
4. Commit with format `vX.Y.Z — <summary>`.
5. `npm publish` (requires npm auth).
